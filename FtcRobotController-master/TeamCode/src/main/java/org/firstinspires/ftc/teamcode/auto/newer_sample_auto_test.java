...
public class newer_sample_auto_test extends OpMode{
    ...
    private final Pose basket = new Pose(15.3, 128, Math.toRadians(-45));
    ...
    private final Pose park =  new Pose(30, 128, Math.toRadians(0));

    private PathChain preload, pickUpClose, placeClose, pickUpMid, placeMid, pickUpFar, placeFar, parkChain;
    ...
    private void buildPaths(){
        ...
        parkChain = follower.pathBuilder()
                .addPath(new BezierLine(new Point(basket), new Point(park)))
        ...
        switch (pathState) {
            ...
            case 7: // Wait until the robot is near the parking position
                if (!follower.isBusy()) {
                    ...
                    else if(waitTimer.milliseconds()>1500) {
                        follower.setMaxPower(1);
